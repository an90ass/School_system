...
class MyMainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("Öğrenci puanları düzenleme sayfası")
    ...
    def search_student(self):
            ...
            if not entered_student_number:
                ...
                if class_name is not None:
                    ...
                    msg.exec_()
                    all_idsAndNot = self.db.getStudentPuanlar(self.student_id)
                    if all_idsAndNot is  None:
                            msg = QMessageBox()
                            msg.setIcon(QMessageBox.Warning)
                            msg.setText("Öğrenci için daha önce not girilmedi. Değişiklik yapabilmek için lütfen notları girin.")
                            msg.setWindowTitle("Uyarı")
                            msg.setStyleSheet("color: red;")
                            msg.exec_()
                    else:
                            lesson_ids = [item[1] for item in all_idsAndNot]
                            notlar = [item[2] for item in all_idsAndNot]
                            lesson_names_list = [self.db.getLessonNamesByIdFromLesson(id) for id in lesson_ids]
                            ...
                            num_rows = len(lesson_names_list)
                            num_cols = 3

                            self.table_widget.setRowCount(num_rows)
                            self.table_widget.setColumnCount(num_cols)

                            for row, (lesson_name, lesson_id, not_value) in enumerate(zip(lesson_names_list, lesson_ids, notlar)):
                                ...
                                grade_input.setText(str(not_value))  # Set the grade value
    ...
    def enter_grade(self, row):
        ...
        try:
            ...
            if 0 <= puan <= 100:
                ...
                self.db.editStudentPuanlar(self.student_id, self.ders_id, puan)
                grade_input.clear()
            else:
